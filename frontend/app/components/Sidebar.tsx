...
export default function Sidebar({ view, setView, saldo, userId, userData }: any) {
  ...
  return (
    <>
      {/* ================================================= */}
      {/* DESKTOP SIDEBAR — só visível em lg+               */}
      {/* ================================================= */}
      <aside className="hidden lg:flex w-56 bg-black/40 backdrop-blur-2xl border-r border-white/10 p-4 flex-col gap-2 fixed inset-y-0 left-0 h-screen overflow-y-auto custom-scrollbar pt-8 pb-4 z-40 shadow-[4px_0_24px_rgba(0,0,0,0.4)]">
          ...
          {userId && (
            <div className="mt-4 bg-black/30 backdrop-blur-md p-2.5 rounded-xl border border-white/10 relative overflow-hidden group hover:border-emerald-500/30 transition-colors">
              ...
              <div className="flex justify-between items-center mb-1 relative z-10">
                <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest group-hover:text-white transition-colors">Nível {currentLevel}</span>
                <span className="text-[9px] font-black text-emerald-400 drop-shadow-[0_0_5px_rgba(16,185,129,0.5)]">{percentagemProgresso}%</span>
              </div>
              <div className="w-full h-1 bg-black/50 rounded-full overflow-hidden relative z-10 border border-white/5 shadow-inner">
                <div className="h-full bg-gradient-to-r from-emerald-600 to-emerald-400 shadow-[0_0_10px_rgba(16,185,129,0.8)] transition-all duration-1000 ease-out relative" style={{ width: `${percentagemProgresso}%` }}>
                  <div className="absolute top-0 right-0 w-2 h-full bg-white/50 blur-[1px]"></div>
                </div>
              </div>
              <p className="text-[7px] text-zinc-500 font-mono mt-1 text-right font-bold uppercase tracking-wider group-hover:text-zinc-300 transition-colors">{currentXp} / {xpTotalNecessario} XP</p>
            </div>
          )}
        </div>
        ...
        <div className="mt-auto pt-4">
          {userId ? (
            <div className="bg-gradient-to-b from-black/60 to-black/40 backdrop-blur-md p-3 rounded-2xl border border-white/10 flex flex-col items-center shadow-xl relative overflow-hidden">
              ...
              <div onClick={() => setView('profile')} className="flex items-center gap-2 w-full bg-white/5 p-2 rounded-xl mb-3 border border-white/5 overflow-hidden cursor-pointer hover:bg-white/10 hover:border-emerald-500/50 transition-all group relative z-10">
                <div className="relative">
                  <img src={userData?.avatar || "https://avatars.steamstatic.com/fef49e7fa7e1997310d705b2a6158ff8dc1cdfeb_full.jpg"} alt="Avatar" className="w-8 h-8 rounded-lg border border-emerald-500/50 shrink-0 group-hover:scale-105 transition-transform" />
                ...
                <div className="flex flex-col overflow-hidden">
                  ...
                  <span className="text-[8px] text-emerald-400 uppercase tracking-widest flex items-center gap-1 group-hover:text-emerald-300">Ver Perfil</span>
                </div>
              </div>
              ...
              <div className="flex gap-2 w-full mt-3 relative z-10">
                ...
                <button onClick={() => { localStorage.removeItem('userId'); window.location.reload(); }} className="px-2 py-1.5 bg-white/5 hover:bg-red-500/20 hover:border-red-500/50 transition-all text-zinc-400 hover:text-red-400 text-[9px] font-black rounded-md uppercase border border-white/5 hover:-translate-y-0.5" title="Terminar Sessão">Sair</button>
              </div>
            </div>
          ) : (
            <button onClick={() => window.location.href = 'https://sweet-7ifa.onrender.com/auth/steam'} className="w-full py-4 px-3 bg-gradient-to-br from-[#171a21] to-[#0a0c10] hover:from-[#2a303c] hover:to-[#171a21] transition-all rounded-2xl border border-white/10 flex flex-col items-center gap-2 group shadow-[0_0_20px_rgba(23,26,33,0.8)] relative overflow-hidden">
      ...
      {/* ================================================= */}
      {/* MOBILE HEADER — fixo no topo, só em mobile        */}
      {/* ================================================= */}
      <header className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-[#0b0b0d]/95 backdrop-blur-xl border-b border-white/10 px-4 h-14 flex items-center justify-between">
        <h1 className="text-base font-black italic text-emerald-400 tracking-tighter">SWEET DROP</h1>
        <div className="flex items-center gap-3">
          {userId ? (
            <>
              <div className="flex items-center gap-1.5 bg-black/40 border border-white/10 rounded-xl px-3 py-1.5">
                <span className="text-white font-black text-sm">{Number(saldo).toFixed(2)}</span>
                <span className="text-emerald-500 font-black text-sm">€</span>
              </div>
              <button onClick={() => setModalAberto(true)} className="w-8 h-8 bg-emerald-500 hover:bg-emerald-400 text-black font-black text-lg rounded-xl flex items-center justify-center transition-all">+</button>
              <img src={userData?.avatar || '/skins/glock.png'} onClick={() => setView('profile')} className="w-8 h-8 rounded-xl border border-emerald-500/50 cursor-pointer" alt="avatar" onError={(e: any) => { e.currentTarget.src = '/skins/glock.png'; }} />
            </>
          ) : (
            <button onClick={() => window.location.href = 'https://sweet-7ifa.onrender.com/auth/steam'} className="flex items-center gap-2 py-2 px-3 bg-[#171a21] border border-white/10 text-white text-[10px] font-black rounded-xl uppercase">
              <svg className="w-4 h-4 fill-current" viewBox="0 0 496 512"><path d="M248 8C111.03 8 0 119.03 0 256c0 136.97 111.03 248 248 248s248-111.03 248-248C496 119.03 384.97 8 248 8zm-82.59 361.35c-43.14 0-78.22-35.08-78.22-78.22s35.08-78.22 78.22-78.22 78.22 35.08 78.22 78.22-35.08 78.22-78.22 78.22zm219.05-87.1l-146.43 43.5c-4.28-7.7-10.7-14.12-18.4-18.4l43.5-146.43c22.18 5.48 40.85 19.38 52.47 37.38 11.62 18 14.88 39.58 8.86 60.45z"/></svg>
              Login Steam
            </button>
          )}
        </div>
      ...
      {/* ================================================= */}
      {/* MOBILE BOTTOM NAV — fixo no fundo, só em mobile   */}
      {/* ================================================= */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#0b0b0d]/95 backdrop-blur-xl border-t border-white/10">
        <div className="flex items-center justify-around px-2 py-2">
          <button onClick={() => setView('store')} className={`flex flex-col items-center gap-0.5 px-4 py-2 rounded-xl transition-all ${view === 'store' || view === 'opening' ? 'text-emerald-400' : 'text-zinc-600 active:text-white'}`}>
            <span className="text-2xl">🛒</span>
            <span className="text-[9px] font-black uppercase">Store</span>
          </button>
          <button onClick={() => setView('upgrader')} className={`flex flex-col items-center gap-0.5 px-4 py-2 rounded-xl transition-all ${view === 'upgrader' ? 'text-emerald-400' : 'text-zinc-600 active:text-white'}`}>
            <span className="text-2xl">⚖️</span>
            <span className="text-[9px] font-black uppercase">Upgrade</span>
          </button>
          <button onClick={() => setView('battles')} className={`flex flex-col items-center gap-0.5 px-4 py-2 rounded-xl transition-all ${view === 'battles' ? 'text-red-400' : 'text-zinc-600 active:text-white'}`}>
            <span className="text-2xl">⚔️</span>
      ...
      {menuAberto && (
        <>
          <div className="lg:hidden fixed inset-0 z-40 bg-black/60 backdrop-blur-sm" onClick={() => setMenuAberto(false)} />
          <div className="lg:hidden fixed bottom-[60px] left-0 right-0 z-50 bg-[#0e0e11] border-t border-white/10 p-4 animate-in slide-in-from-bottom-2 duration-200">
            
            {/* XP bar no menu mobile */}
            {userId && (
              <div className="bg-black/30 p-3 rounded-xl border border-white/5 mb-4">
                <div className="flex justify-between items-center mb-1.5">
                  <span className="text-[10px] font-black text-zinc-400 uppercase">Nível {currentLevel}</span>
                  <span className="text-[10px] font-black text-emerald-400">{currentXp} / {xpTotalNecessario} XP</span>
                </div>
                <div className="w-full h-1.5 bg-black/50 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-emerald-600 to-emerald-400 rounded-full transition-all" style={{ width: `${percentagemProgresso}%` }}></div>
                </div>
              </div>
            )}

            <div className="grid grid-cols-4 gap-2 mb-4">
              <button onClick={() => { setView('coinflip'); setMenuAberto(false); }} className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all ${view === 'coinflip' ? 'border-amber-500/50 bg-amber-500/10 text-amber-400' : 'border-white/5 bg-white/5 text-zinc-400'}`}>
                <span className="text-2xl">🪙</span>
                <span className="text-[9px] font-black uppercase">Coinflip</span>
              </button>
              <button onClick={() => { setView('giveaways'); setMenuAberto(false); }} className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all ${view === 'giveaways' ? 'border-purple-500/50 bg-purple-500/10 text-purple-400' : 'border-white/5 bg-white/5 text-zinc-400'}`}>
                <span className="text-2xl">🎁</span>
            ...
            {userId && (
              <button onClick={() => { localStorage.removeItem('userId'); window.location.reload(); }} className="w-full py-3 bg-red-500/10 border border-red-500/20 text-red-400 text-[10px] font-black rounded-xl uppercase tracking-widest">
                Terminar Sessão
              </button>
            )}
          </div>
      ...
      {modalAberto && <ModalDeposito userId={userId} onClose={() => setModalAberto(false)} />}
