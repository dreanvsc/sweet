...
export default function BattleArena({ batalha, userId, onLeave }: any) {
  ...
  const teveDesempate = resultado?.teveDesempate || false; // 🔥 NOVO: Verifica se houve coinflip

  const [rondaAtual, setRondaAtual] = useState(0);
  const [terminou, setTerminou] = useState(false);
  const [isSpinning, setIsSpinning] = useState(true);
  
  // 🔥 ESTADO NOVO: Suspense do Coinflip
  const [mostrandoDesempate, setMostrandoDesempate] = useState(false);
  ...
  useEffect(() => {
    if (rondaAtual >= totalRondas) {
      ...
      // 🔥 LÓGICA DE SUSPENSE PARA O COINFLIP!
      if (teveDesempate) {
        setMostrandoDesempate(true);
        // Espera 5 segundos a fazer suspense com o texto de empate
        setTimeout(() => {
          setMostrandoDesempate(false);
          setTerminou(true); // Só agora mostra o Vencedor!
        }, 5000);
      } else {
        setTerminou(true); // Se não houve empate, termina normalmente
      }
      return;
    }
  ...
  const getCardStyle = (jId: any) => {
    ...
    // 🔥 ESTILO DURANTE O SUSPENSE DO COINFLIP
    if (mostrandoDesempate) {
      if (empateIds.includes(jId)) return 'border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.5)] animate-pulse opacity-100 z-10'; // Quem empatou brilha a azul
      return 'border-transparent opacity-20 grayscale'; // Os outros perdem a cor
    }
    
    // 🔥 ESTILO DO VENCEDOR FINAL
    ...
    <div className="w-full max-w-7xl mx-auto p-4 animate-in fade-in">
      ...
      <div className="flex justify-between items-end mb-8 border-b border-white/5 pb-4">
        <div>
           ...
           {!terminou && !mostrandoDesempate && <p className="text-emerald-500 font-bold uppercase tracking-widest text-xs mt-1">A Abrir: {caixaAtual?.nome} (Ronda {rondaAtual + 1}/{totalRondas})</p>}
        </div>
      ...
      {/* PLACARD FINAL DE VITÓRIA (SÓ APARECE DEPOIS DE TERMINAR) */}
      {terminou && !mostrandoDesempate && (
        <div className="mt-8 text-center flex flex-col items-center justify-center animate-in slide-in-from-bottom-8">
            <div className="bg-amber-500/10 border border-amber-500/30 px-20 py-8 rounded-3xl shadow-[0_0_50px_rgba(245,158,11,0.2)]">
              <p className="text-amber-500 font-black tracking-widest text-xs mb-1 uppercase">
                {teveDesempate ? "VENCEU O COINFLIP DE DESEMPATE!" : "O Vencedor Limpou Tudo!"}
              </p>
